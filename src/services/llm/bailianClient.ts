...
export class BailianLLMClient implements LLMClient {
  ...
  async generateItinerary(input: GenerateItineraryInput): Promise<GeneratedItinerary> {
    ...
    const prompt = [
      ...
      '餐饮要求（关键约束）：\n- 仅“中间天数”（除第一天与最后一天外）必须包含午餐与晚餐两个独立的餐饮段（type=food），早餐可选；\n- 第一日如抵达时间晚于午餐或晚餐时间窗口（如午餐 11:30-13:30、晚餐 18:00-20:30），可不在行程中规划当日相应餐饮；\n- 最后一日如返程时间早于午餐或晚餐时间窗口（如午餐 11:30-13:30、晚餐 18:00-20:30），可不在行程中规划当日相应餐饮；\n- 每个餐饮段需包含：具体餐厅名称与地址（location）、时间段（timeRange，建议午餐 11:30-13:30、晚餐 18:00-20:30，可适当调整）、推荐菜（notes 中说明）、人均消费 costEstimate（CNY，数字）；\n',
      ...
      '目的地语义（重要）：\n- destination 字段始终指“旅游目的地城市”，贯穿整个行程保持不变；\n- 最后一天不要将 destination 误认为出发城市，即使从目的地离开，destination 仍是旅游城市。',
      '目的地内安排约束（关键）：\n- 除跨城交通段（type=transport）外，所有段的活动（餐饮/景点/娱乐/住宿/购物等）必须安排在 destination 城市内的具体地点；\n- 禁止在出发地（origin）城市安排任何活动（餐饮/景点/娱乐/住宿/购物等）；\n- 第一天仅安排从 origin 到 destination 的交通（以及到达后的 destination 内活动），不要在 origin 城市安排停留或活动；\n- 最后一天将“destination 回到 origin”的返程交通作为当天最后一段，其余安排仍在 destination 城市；',
      '位置字段约束（重要）：\n- 非交通段的 location 必须为 destination 城市内的具体地点；\n- 跨城交通段（如从 origin 到 destination 或从 destination 回到 origin）中，location 只填写目的地城市的具体站点（destination 城市的车站/机场/地标），不要写“origin-destination”或“destination-origin”的组合字符串；\n- 示例：origin=南昌，destination=南京：第一天到达与最后一天离开，transport 段的 location 均填写“南京南站”（destination 城市的站点）；其他段落 location 均为南京市内具体地点。',
      '预算与一致性（必须遵守）：\n- 每段 costEstimate 为“人均 CNY 数字”，不可为占位符或范围；\n- 为每一天计算 dayBudget（总额=当日所有分段 costEstimate 之和× party_size），允许比加和高出 0–10% 作为“杂项”，但不可低于分段加和；\n- 总 budget=所有 dayBudget 的加和，允许比加和高出 0–10%，但不可低于加和；\n- 明确考虑同行人数 party_size：所有 dayBudget 与总 budget 均为该人数的总额（而非人均）；\n- 每个 day 对象请包含 dayBudget 字段（数字）。',
      '预算目标（若提供）：\n- 若存在 budget_hint（数字）且判断为合理（结合目的地与常见价格，人均每天通常 200–800 CNY；明显低于人均每天100 或高于人均每天3000 视为不合理），请以该预算为总额目标，安排行程使最终总 budget 与 budget_hint 的误差≤10%；必要时优先调整餐饮/娱乐/购物等可选项的成本或选择替代方案，住宿与跨城交通的成本需保持现实；\n- 若 budget_hint 不合理或未提供，则自行估算预算并满足上述“分段→每日→总计”的一致性关系。',
