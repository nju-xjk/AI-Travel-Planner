...
export class BailianLLMClient implements LLMClient {
  ...
  async generateItinerary(input: GenerateItineraryInput): Promise<GeneratedItinerary> {
    ...
    const prompt = [
      ...
      '位置字段约束（重要）：\n- 跨城交通段（如从 origin 到 destination 或从destination 回到 origin）中，location 只填写目的地城市的具体地点（destination 城市的车站/机场/地标），不要写“origin-destination”或“destination-origin”的组合字符串；\n- 示例：origin=南昌，destination=南京：第一天到达与最后一天离开，transport 段的 location 均填写“南京南站”（destination城市的站点）。',
